...
def detectar_top_1722(
    ...
    for conta in contas_com_top1722:
        ...
        if abs(diferenca) < 0.005:
            ...
            # v5.26: Mesmo com diferença grande (> 3.5%), AGRUPA. Antes pulava e jogava
            # 1.345 linhas em divergência (alarme falso). Agora consome as linhas e
            # registra com status diferente pra usuário investigar a causa.
            com_diff.append({
                "id_grupo": id_grupo,
                "conta": conta,
                "qtd_creditos_banco": len(creditos_cartao),
                ...
                "diferenca": diferenca,
                "percentual_diferenca": round(pct_diff * 100, 3),
                "status": "TOP 1722 com Diferença Grande (agrupado, INVESTIGAR)",
                "motivo": (
                    f"Sankhya R$ {soma_sankhya:.2f} × Banco R$ {soma_banco:.2f}. "
                    f"Diferença R$ {abs(diferenca):.2f} ({pct_diff*100:.2f}%) excede o "
                    f"limite de {TOLERANCIA_REL_DIFERENCA*100:.1f}%. "
                    f"Possível causa: período diferente entre banco e Sankhya, ou "
                    f"lançamentos faltando. Linhas consumidas mesmo assim."
                ),
            })
            # v5.26: consome linhas pra elas saírem de Pendentes/Divergência
            indices_banco_casados.update(int(i) for i in creditos_cartao["_idx_banco"].tolist())
            indices_sankhya_casados.update(int(i) for i in sis_conta["_idx_sis"].tolist())
