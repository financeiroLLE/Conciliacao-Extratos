    ...
    for row_idx, (_, row) in enumerate(df.iterrows(), start=inicio_linha + 1):
        for col_idx, col_name in enumerate(df.columns, start=1):
            ...
            if pd.isna(val):
                ...
                # v5.9: sanitiza bytes — openpyxl quebra com "cannot be used in worksheets"
                try:
                    cell.value = val.decode("utf-8", errors="replace")
                except Exception:
                    cell.value = repr(val)[:255]
            else:
                # v5.9: garante que o valor seja serializ\u00e1vel pelo openpyxl
                # (alguns parsers podem retornar tipos ex\u00f3ticos)
                try:
                    cell.value = val
                except Exception:
...
def _safe_cell_value(val):
    """v5.9: sanitiza valor antes de escrever em c\u00e9lula do openpyxl.
    Evita erro 'cannot be used in worksheets' com bytes/tipos ex\u00f3ticos."""
