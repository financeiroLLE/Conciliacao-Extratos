...
# v6.14 (11/09/2026): CSS override agora INLINE abaixo, sem depender do módulo
# separado src/style_light.py (que estava com cache do Streamlit Cloud, não pegava
# nova versão nem depois de Reboot). Manter esta linha comentada.
...
# v6.17: constantes de paleta usadas pelo CSS v6.12 concatenado abaixo
# ============================================================
_NAVY = "#0F1F46"
_NAVY_2 = "#0A1730"
...
/* ============================================================================
   FASE A · v6.17 — CSS NOVO CONCATENADO NO MESMO st.html() (fix Streamlit)
...
/* TESTE DIAGNOSTICO v6.17 - FAIXA VERMELHA */
html body::before {{
    content: "CSS v6.17 - FAIXA CSS OK" !important;
...
st.error("🔴 DIAGNOSTICO v6.17 — se voce esta vendo esta mensagem VERMELHA, o codigo NOVO do app.py esta rodando (Streamlit Cloud pegou a versao nova)")
...
def _render_conta70_mapa_recebimentos():
    ...
    # v5.24: "Mostrar" amarelo. O F12 revelou que a caixa é react-aria (não
    # data-baseweb=select, que eu vinha mirando) e que o container tem a classe
    # estável st-key-c70_mapa_filtro. Miramos nela, no [role=group] (a caixa).
    st.markdown(
        "<style>"
        '.st-key-c70_mapa_filtro [role="group"]{background-color:#FAC318!important;border-color:#d9a800!important}'
        '.st-key-c70_mapa_filtro [role="group"] *{color:#041747!important}'
        '.st-key-c70_mapa_filtro input{color:#041747!important;background:transparent!important;-webkit-text-fill-color:#041747!important}'
...
def _render_conta70_casamento_numeracao():
    ...
    if est.empty:
        ...
        with st.container(key="c70filtros"):
            st.markdown(
                "<style>"
                '.st-key-c70_fprio [role="group"],.st-key-c70_fdiag [role="group"],'
                '.st-key-c70_fbanco [role="group"],.st-key-c70_fcol [role="group"],'
                '.st-key-c70_busca [role="group"],.st-key-c70_busca [data-baseweb="base-input"],'
                '.st-key-c70_busca [data-baseweb="input"],'
                '.st-key-c70_ford [role="group"]'
                '{border-left:4px solid #FAC318!important}'
                "</style>",
                unsafe_allow_html=True,
            )
            # v5.86: adicionamos a coluna de ordenação (fc_ord). Default:
            # "Valor (pares juntos)" — agrupa mesma cifra absoluta lado a lado
            # pra facilitar achar par receita+despesa no atrelamento manual.
            fc_ord, fc1, fc2, fc3, fc4, fc5 = st.columns([1.3, 1, 1.2, 1, 1.3, 1.6])
            f_ord = fc_ord.selectbox(
                "Ordenar por",
                ["Valor (pares juntos)", "Data (mais recente)", "Data (mais antiga)",
                 "Valor decrescente", "Diagnóstico"],
                key="c70_ford",
            )
        ...
        # v5.86: aplica a ordenação escolhida. "Valor (pares juntos)" é o
        # default novo — agrupa mesma cifra absoluta lado a lado pra facilitar
        # achar despesa+receita do mesmo evento (ex.: -400 e +400 do mesmo
        # estorno ficam adjacentes).
        _v_abs = view["valor"].abs().round(2)
        _dt = pd.to_datetime(view["data"], errors="coerce")
        _rd_ord = view["receita_despesa"].astype(str).str.upper()
        if f_ord == "Valor (pares juntos)":
            view = view.assign(_ord_v=_v_abs, _ord_rd=_rd_ord.map(lambda x: 0 if "DESPESA" in x else 1), _ord_d=_dt)
            view = view.sort_values(by=["_ord_v", "_ord_rd", "_ord_d"], ascending=[False, True, True])
            view = view.drop(columns=["_ord_v", "_ord_rd", "_ord_d"])
        elif f_ord == "Data (mais recente)":
            view = view.assign(_ord_d=_dt).sort_values(by="_ord_d", ascending=False).drop(columns=["_ord_d"])
        elif f_ord == "Data (mais antiga)":
            view = view.assign(_ord_d=_dt).sort_values(by="_ord_d", ascending=True).drop(columns=["_ord_d"])
        elif f_ord == "Valor decrescente":
            view = view.assign(_ord_v=_v_abs).sort_values(by="_ord_v", ascending=False).drop(columns=["_ord_v"])
        elif f_ord == "Diagnóstico":
            view = view.sort_values(by="diagnostico", ascending=True)
